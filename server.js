...
// Ping di mantenimento WebSocket setInterval(() => { wss.clients.forEach(client => { if (client.readyState === WebSocket.OPEN) { client.ping(); } }); }, 20000);
const TIKTOK_USERNAME = 'manolita772'; // Cambia qui con lo username esatto
function connectToTikTok() { const tiktokLiveConnection = new WebcastPushConnection(TIKTOK_USERNAME);
tiktokLiveConnection.connect() .then(state => { console.log('CONNESSO A TIKTOK LIVE! RoomId: ' + state.roomId); }) .catch(err => { console.log('TikTok non in Live o errore di firma. Riprendo tra 10s...'); setTimeout(connectToTikTok, 10000); });
